class ClosestRestaurantResponse {
  String? message;
  List<closestData>? data;
  bool? status;

  ClosestRestaurantResponse({this.message, this.data, this.status});

  ClosestRestaurantResponse.fromJson(Map<String, dynamic> json) {
    message = json['message'];
    if (json['data'] != null) {
      data = <closestData>[];
      json['data'].forEach((v) {
        data!.add(new closestData.fromJson(v));
      });
    }
    status = json['status'];
  }
...
class closestData {
  ...
  closestData.fromJson(Map<String, dynamic> json) {
    id = json['id'];
    name = json['name'];
